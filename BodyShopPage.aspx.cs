...
public partial class BodyShopPage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        //set website title
        var page = (Page)HttpContext.Current.Handler;
        page.Title = "The Body Shop's blog";

        //showing current system time
        TimeLabel.Text = DateTime.Now.ToString("dd/MM/yyyy H:mm:ss");

        //read the user entry for The Body Shop
        StreamReader sr = new StreamReader(Server.MapPath("~") + "/Files/BodyShopUsersEntry.txt", true);
        BodyShopTextBox.Text = sr.ReadLine().ToString(); //read the file, and then print it out to user
