...
public partial class FarmersPage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        //set website title
        var page = (Page)HttpContext.Current.Handler;
        page.Title = "Famers's blog";

        //showing current system time
        TimeLabel.Text = DateTime.Now.ToString("dd/MM/yyyy H:mm:ss");

        //read the user entry for Farmers
        StreamReader sr = new StreamReader(Server.MapPath("~") + "/Files/FarmersUserEntry.txt", true);
        FarmersTextBox.Text = sr.ReadLine(); //read the file, and then print it out to user
